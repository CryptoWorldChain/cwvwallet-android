...
public class LineChartView extends View {
    ...
    private int lineColor = Color.parseColor("#e1b631");//曲线颜色
    ...
    private int tableColor = Color.parseColor("#FFFFFF");//表格线颜色
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (CheckedUtils.INSTANCE.nonEmpty(dataList)) {
            stepSpace = (getWidth() - stepStart) / (dataList.size() + 1);
...
//            tablePath.lineTo(tableEnd, startHeight);
    ...
    private void drawRulerXValue(Canvas canvas) {
        ...
        for (int i = 0; i < linePoints.length; i++) {
            ...
            drawRulerXText(canvas, dataList.get(i).xText, linePoints[i].x, 0);
        }
    }
